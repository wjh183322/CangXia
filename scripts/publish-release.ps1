...
try {
    ...
    if ((git branch --show-current) -ne 'codex/nas' -or $package.name -ne 'cangxia-nas') { throw '本脚本仅用于 codex/nas 分支的 NAS 版。' }
    ...
    $exe = "release/CangXia-NAS-$version-Windows-x64.exe"
    ...
    Set-Content -LiteralPath $hashFile -Value "$hash  CangXia-NAS-$version-Windows-x64.exe" -Encoding utf8
    git push origin HEAD $tag
    if ($LASTEXITCODE -ne 0) { throw '源码或标签推送失败。' }
    & $ghPath release create $tag $exe $hashFile --repo $Repository --verify-tag --title "藏匣 NAS 版 v$version" --notes-file $notes --prerelease --latest=false
