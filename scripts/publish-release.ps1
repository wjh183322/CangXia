...
try {
    ...
    if ((git branch --show-current) -ne 'codex/local' -or $package.name -ne 'cangxia-local') { throw '本脚本仅发布独立本地版分支。' }
    ...
    $exe = "release/CangXia-Local-$version-Windows-x64.exe"
    $notes = "docs/releases/local-v$version.md"
    ...
    Set-Content -LiteralPath $hashFile -Value "$hash  CangXia-Local-$version-Windows-x64.exe" -Encoding utf8
    git push origin HEAD $tag
    if ($LASTEXITCODE -ne 0) { throw '源码或标签推送失败。' }
    & $ghPath release create $tag $exe $hashFile --repo $Repository --verify-tag --title "藏匣本地版 $version 预览" --notes-file $notes --prerelease --latest=false
