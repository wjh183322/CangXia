...
export function validateAuth(input){
  ...
  const cookies=input.cookies.filter(c=>c&&typeof c.name==='string'&&typeof c.value==='string'&&['douyin.com','www.douyin.com'].includes(String(c.domain||'').replace(/^\./,''))).map(c=>{
    if(!/^[!#$%&'*+.^_`|~\w-]+$/.test(c.name)||/[\r\n\0]/.test(c.value)||c.value.length>16384)throw new Error('登录配置含无效字段');
    return {...c,domain:c.domain,path:c.path||'/'};
  });
  ...
  return {cookies,userAgent:input.userAgent,source:['chrome','edge','config'].includes(input.source)?input.source:'config'};
}
