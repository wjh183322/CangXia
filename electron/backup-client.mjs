...
export class BackupClient{
 ...
 async prepare(signal){
  ...
  }return {entries:[...map.values()],files};
 }
 ...
 acceptCommit(pending,revision){
  this.applying=true;try{for(const e of pending.request.changes){if(e.body===null)delete this.meta.baseline[recordId(e)];else this.meta.baseline[recordId(e)]=contentHash(e.body);if(e.table==='downloads'){if(e.body)this.store.put('backup_downloads',e.key,e.body);else this.store.db.run('DELETE FROM backup_downloads WHERE id=?',[e.key]);}}this.store.save();}finally{this.applying=false;}
 ...
 async sync(){
  ...
  this.syncing=(async()=>{this.status.phase='syncing';this.status.message='正在后台同步到 NAS，本机可以继续使用';this.emit();try{
   ...
   const needed=new Set(changes.filter(e=>e.table==='downloads'&&e.body).flatMap(e=>e.body.assets.map(a=>a.sha256))),sent=new Set();let index=0;
