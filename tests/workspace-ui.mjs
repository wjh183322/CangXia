...
try{
 ...
 const failedCover=document.querySelector('.work-card img');await act(async()=>{failedCover.dispatchEvent(new Event('error'));await settle();});assert.equal(failedCover.style.display,'none');await click('刷新文件状态');const retriedCover=document.querySelector('.work-card img');check('refresh retries a previously hidden failed cover',retriedCover!==failedCover&&retriedCover.style.display!=='none'&&retriedCover.src!==failedCover.src);
