...
void app.whenReady().then(async()=>{
 ...
 try{
  ...
  const retained=store.all('works').length;await collector.logout();check('logout closes real background Chrome and removes only its private login profile',!browser.connection&&!fs.existsSync(path.join(root,'browser','chrome'))&&store.all('works').length===retained&&store.getSetting('browserAccountKey')==='uid:123');
