  ...
  return {name:'local-ui-test-only',apply:'serve',configureServer(server){
    server.middlewares.use((req,res,next)=>{
      ...
      if(req.url==='/__test__/snapshot'){
        ...
        const snapshot={works,collections:[{id:'__all__',name:'收藏',added:true,count:30,complete:true},{id:'9',name:'界面测试收藏夹',added:true,count:30,complete:true},{id:'10',name:'尚未添加（测试）',added:false,count:0}],members:{__all__:works.map(w=>w.id),'9':works.map(w=>w.id)},root:'界面测试目录（非账号数据）',account:null,version:'0.1.3',collector:{phase:'done',message:'界面测试数据 · 30 个虚构条目'},queue:{jobs:[],paused:false}};
