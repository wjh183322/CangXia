...
export function BackupPanel({storage={},busy,api,onUpdated,onImport,onConflict}){
 ...
 return <section className="backup-panel"><div className={`nas-status ${!storage.writable?'warning':''}`} role="status"><strong>{labels[storage.phase]||'尚未连接'}</strong><span>{storage.message}</span>{storage.progress&&<span>{storage.progress}</span>}</div>
